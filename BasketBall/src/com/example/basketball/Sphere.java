...
public class Sphere {
	...
	private final String vertexShaderCode = "vec4 lightDir = vec4(1, 1, 1, 0); "
	...
	private final String fragmentShaderCode = "precision mediump float;"
			...
			+ "   vec4 lightS = vec4(1.0, 0.0, 1.0, 1.0); "
			+ // the specular highlight is redish
			"   vec3 Nn = normalize(varyingNormal); "
			+ "   vec3 Ln = normalize(lightDir); "
			+ "   vec4 diffuse = kd* light * max(dot(Nn, Ln), 0.0); "
			+ "   vec3 Ref = reflect(Nn, Ln); "
			+ // get the reflectance vector;
			"   float dotV = max(dot(Ref, normalize(varyingPos)), 0.0); "
			+ // since we are in eye space, the eye position is at 0, 0, 0, so
				// the view direction is simply (varyingPos- (0, 0, 0))
			"   vec4 specular = lightS*ks*pow(dotV, Ns); "
			//+ "	gl_FragColor = diffuse + specular; " +
			+ "   gl_FragColor = diffuse*varyingColor + specular; "
			+ "}";
	...
	public final float[] savedViewMatrix = new float[16];
	...
	private void createSphere(double r, int lats, int longs) {
		...
		// there are lats*longs number of quads, each requires two triangles
		// with six vertices, each vertex takes 3 floats;
		...
		for (i = 0; i < lats; i++) {
			...
			for (j = 0; j < longs; j++) {
				...
				// in this case, the normal is the same as the vertex, plus the
				// normalization;
				for (int kk = -9; kk < 9; kk++) {
					normals[triIndex * 9 + kk] = vertices[triIndex
							* 9 + kk];
					colors[triIndex * 9 + kk] = color[(kk+9)%3];
				}
				triIndex++;
			}
			// glEnd();
	...
	public Sphere(double r, int lats, int longs, float[] color, float x, float y) {	
		this.color = color;
		...
		int vertexShader = RenderEngine.loadShader(GLES20.GL_VERTEX_SHADER,
				vertexShaderCode);
		checkShaderError(vertexShader);
		int fragmentShader = RenderEngine.loadShader(GLES20.GL_FRAGMENT_SHADER,
				fragmentShaderCode);
	...
	public void draw(float[] mvpMatrix, float[] normalMat, float[] mvMat) { // pass
																			// in
																			// the
																			// calculated
																			// transformation
		...
		GLES20.glVertexAttribPointer(mColorHandle, COORDS_PER_VERTEX,
				GLES20.GL_FLOAT, false, vertexStride, colorBuffer);
